...
import { loginAction, castVoteAction } from './actions';
...
export default function POCPage() {
  ...
  const handleVote = async (candidateId: number) => {
    setLoading(true);
    const result = await castVoteAction(1, candidateId); // Election ID 1
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 font-sans text-slate-800">
      <h1 className="text-3xl font-bold mb-2 text-blue-700">POC: Secure Voting Architecture</h1>
      <p className="mb-8 text-slate-600 border-b pb-4">
        หน้าสาธิตการทำงานแบบ Server-Action + Database จริง เพื่อแก้ปัญหาความปลอดภัย
      </p>
      ...
      {step === 'LOGIN' && (
        <div className="bg-white p-6 shadow-lg rounded-xl border border-slate-200">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
                <label className="block text-sm font-medium mb-1">Student ID (Mock: 1234)</label>
                <input
                    type="text"
                    value={studentId}
            ...
            <div>
                <label className="block text-sm font-medium mb-1">National ID Hash (Mock: hashed_secret_123)</label>
                <input
                    type="password"
                    value={nationalId}
          ...
          <p className="mt-4 text-xs text-slate-500">
            * ระบบจะไม่ส่งไฟล์ data.json มาที่ client อีกต่อไป แต่จะส่งแค่ Credentials ไปตรวจสอบที่ Server
          </p>
        </div>
      )}
        ...
        <div className="bg-white p-6 shadow-lg rounded-xl border border-slate-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
                ...
                <div className="text-sm text-slate-500">นาย ก. (Transaction Test)</div>
            </button>
            <button
            ...
            >
                ...
                <div className="text-sm text-slate-500">นาง ข. (Transaction Test)</div>
            </button>
          </div>
          <p className="mt-6 text-xs text-slate-500">
            * เมื่อกดโหวต ระบบจะใช้ Database Transaction เพื่อ:<br/>
            1. เช็คว่าเคยโหวตหรือยัง (ตาราง vote_history)<br/>
